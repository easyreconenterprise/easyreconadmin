...
//         console.log("Categorized Data Response:", categorizedResponse.data);
...
const ParentMapped = () => {
  ...
  useEffect(() => {
    const fetchLedgerAndStatement = async () => {
      if (!currentSession) return; // Ensure session data exists

      const accountId = currentSession.account; // From the session context
      const switchId = currentSession._id; // From the session context
      const token = localStorage.getItem("jwtToken");
      ...
      try {
        ...
        // Fetch ledger data (Assumed you have a ledger endpoint)
